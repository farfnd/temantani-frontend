...
const ProjectList = () => {
    const [projects, setProjects] = useState(() => {
        const cachedOngoingProjects = localStorage.getItem('cachedOngoingProjects');
        const cacheExpiryO = localStorage.getItem('cacheExpiryO');

        if (cachedOngoingProjects && cacheExpiryO && Date.now() < parseInt(cacheExpiryO)) {
            return JSON.parse(cachedOngoingProjects);
        }
        return [];
    });
    ...
    const fetchProjects = async () => {
        try {
            ...
            const cachedOngoingProjects = localStorage.getItem('cachedOngoingProjects');
            let cacheExpiryO = localStorage.getItem('cacheExpiryO');

            // Check if cached data is still valid
            if (cachedOngoingProjects && cacheExpiryO && Date.now() < parseInt(cacheExpiryO)) {
                setProjects(JSON.parse(cachedOngoingProjects));
                setLoading(false);
                return;
            }

            const response = await fetch(`${config.api.projectService}/projects?filter[status]=ONGOING`, {
                method: 'GET',
                headers: {
            ...
            cacheExpiryO = Date.now() + 3600000;
            localStorage.setItem('cachedOngoingProjects', JSON.stringify(updatedProjects));
            localStorage.setItem('cacheExpiryO', cacheExpiryO.toString());
        } catch (error) {
            console.error(error);
                ...
                <>
                    <Row className="row-cols-1 row-cols-md-2 row-cols-lg-4 pb-3" xs={1} md={2} lg={4} gap={16}>
                        {currentProjects.map((project) => (
                            <Col key={project.id}>
                                <Link to={`/admin/projects/ongoing/${project.id}`}>
